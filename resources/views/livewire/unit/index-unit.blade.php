                    ...
                    <form method="GET">
                        <div class="row m-3">
                            ...
                            <div class="col-lg-4 text-end">
                                <a href="{{ route('create.unit') }}" class="btn btn-primary border" title="Create New"><i class="bx bx-plus"></i> New</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="table-responsive text-nowrap mx-4">
                    <table class="table table-borderless table-striped">
                        ...
                        <tbody>
                            @if($units->count() > 0)
                            @php
                            $no = ($units->currentPage() - 1) * $units->perPage() + 1
                            @endphp
                            @foreach ($units as $unit)
                            <tr>
                                <td class="text-start">
                                    <a href="{{ route('edit.unit', $unit->id) }}" class="dropdown-item">
                                        {{ $unit->name }}
                                    </a>
                                </td>
                                <td class="text-end">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <button class="dropdown-item text-danger" wire:click="showDialogDelete('{{ $unit->id }}')"><i class="bx bx-trash me-1"></i> Delete</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
