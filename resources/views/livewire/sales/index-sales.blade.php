                    ...
                    <form method="GET">
                        <div class="row m-3">
                            ...
                            <div class="col-lg-4 text-end">
                                <a href="{{ route('create.sales') }}" class="btn btn-primary border" title="Create New"><i class="bx bx-plus"></i> New</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="table-responsive text-nowrap mx-4">
                    <table class="table table-borderless table-striped">
                        ...
                        <tbody>
                            @if($saleses->count() > 0)
                            @php
                            $no = ($saleses->currentPage() - 1) * $saleses->perPage() + 1
                            @endphp
                            @foreach ($saleses as $sales)
                            <tr>
                                <td class="text-start">
                                    <a href="{{ route('edit.sales', $sales->id) }}" class="dropdown-item">
                                        {{ $sales->name }}
                                    </a>
                                </td>
                                <td class="text-end">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <button class="dropdown-item text-danger" wire:click="showDialogDelete('{{ $sales->id }}')"><i class="bx bx-trash me-1"></i> Delete</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
<script>
    ...
    window.addEventListener('showDialogDelete', (event) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // jalan function salesConfirmAcrion di backend
