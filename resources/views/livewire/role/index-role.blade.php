                    ...
                    <form method="GET">
                        <div class="row m-3">
                            ...
                            <div class="col-lg-4 text-end">
                                <a href="{{ route('create.role') }}" class="btn btn-primary border" title="Create New Role"><i class="bx bx-plus"></i> New</a>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="badge bg-label-primary me-1 text-capitalize">{{ $this->getFormattedTimestamp($role->created_at) }}</span></td>
                                <td><span class="badge bg-label-primary me-1 text-capitalize">{{ $this->getFormattedTimestamp($role->updated_at) }}</span></td>
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" style="">
                                            <a class="dropdown-item" href="{{ route('edit.role', $role->id) }}"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                                            @if($role->name != 'Superadmin')
                                            <button class="dropdown-item text-danger" wire:click="showDialogDelete('{{ $role->id }}')"><i class="bx bx-trash me-1"></i> Delete</button>
                                            @endif
                                        </div>
                                    </div>
                                </td>
    ...
    window.addEventListener('showDialogDelete', (event) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Livewire.dispatch('confirmDeleteRole', {
                    roleId: event.detail[0].roleId
                });
            } else {
                Swal.fire("Delete Data Role Dibatalkan", "", "info");
            }
        });
    });
</script>